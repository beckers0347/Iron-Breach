"""
SWAP GROUND TO T_ForestGround -- breaks the Iceland-pack streaking for good
================================================================
IRON BREACH / Unreal Engine 5.8

WHY
---
Both T_Iceland_Eroded and T_Iceland_Dirt produced the same directional
wood-grain streaking on the ground -- confirmed by running the dirt version
and it looking identical. That's not a tiling/scale problem, it's the whole
Iceland "eroded terrain" texture family sharing the same wind-swept linear
look, which reads great on jagged mountain rock but wrong on flat open
ground no matter which one of that set gets used.

T_ForestGround is a completely different asset (not part of the Iceland
erosion set) -- a mottled forest-floor texture, no linear grain -- so this
rebuilds M_AI_MountainGround from that instead.

HOW TO RUN IT
-------------
    py "X:/IronBreach/Content/Python/forest_ground_material.py"
...
FOREST_NORMAL = "/Game/Landscaping/IcelandEnviroment/Textures/T_ForestGround/T_ForestGround_N"
FOREST_ROUGH = "/Game/Landscaping/IcelandEnviroment/Textures/T_ForestGround/T_ForestGround_R"


def log(msg):
    print("[ForestGround] %s" % msg)


def build_triplanar(material, basecolor_tex, normal_tex, roughness_tex, tile_world_size=800.0, blend_sharpness=4.0):
    log("Rebuilding %s from T_ForestGround (tile=%.0fcm)..." % (material.get_name(), tile_world_size))
...
def run():
    ...
    normal = AL.load_asset(FOREST_NORMAL)
    rough = AL.load_asset(FOREST_ROUGH)
    if not basecolor:
        log("SKIPPED -- could not load %s" % FOREST_BASECOLOR)
        return
    build_triplanar(ground_mat, basecolor, normal, rough)
