"""
BUILD AN ACTUAL COBBLESTONE PATTERN -- procedural paver grid + grout lines
================================================================
IRON BREACH / Unreal Engine 5.8

WHY
---
The only stone-type texture in the project is T_MossyCreekStones, and a
plain triplanar sample of it just reads as "mottled grey/green ground" --
there's no dark grout/seam lines between individual stones, which is the
one visual cue that actually reads as "cobblestone" or "flagstone" from a
normal play-camera distance. Swapping which texture we sample doesn't fix
that; there's no other stone photoscan in the project to swap to.

So instead of another texture swap, this keeps the same triplanar
MossyCreekStones sampling (basecolor + real roughness map, still no normal
map -- that part's proven clean) for the individual-stone color/roughness
variation, and adds a procedural paver grid on top: world-XY is divided
into square cells, and pixels near each cell edge are darkened into a
grout/moss-line color. That grid is what makes individual stones actually
readable as individual stones instead of a texture blur.

CELL_SIZE_CM=35 with GROUT_WIDTH=0.09 reads as tight cobblestone. If you'd
rather have slate/flagstone instead (bigger, more irregular-reading slabs),
tell me and I'll swap the regular grid for a Voronoi-cell version -- this
first pass uses a regular grid because it's the simpler, safer thing to get
right blind (no in-editor preview from here), and it's a real fix either
way: even as "cobblestone", a regular grid with grout lines will look far
more like a stone path than the flat photoscan blend did.

HOW TO RUN IT
-------------
    py "X:/IronBreach/Content/Python/build_cobblestone_path.py"
...
TILE_WORLD_SIZE = 100.0     # cm -- how big one texture repeat of the stone photoscan is
CELL_SIZE_CM = 35.0         # cm -- size of one paver "stone" in the procedural grid
GROUT_WIDTH = 0.09          # 0..0.5 of a cell -- how wide the dark seam is
GROUT_SOFTNESS = 0.05       # blend width of the seam edge (anti-aliasing)
GROUT_COLOR = (0.045, 0.05, 0.045)   # dark mossy grey-green


def log(msg):
    print("[BuildCobblestone] %s" % msg)


def build_cobblestone(material, basecolor_tex, roughness_tex):
    log("Rebuilding %s -- triplanar stone + procedural grout grid..." % material.get_name())
    MEL.delete_all_material_expressions(material)

    # ---- triplanar setup (same proven pattern, no normal map) ----
    ...
    stone_color = sample_and_blend(basecolor_tex, 0)
    ...
    grout_color_const = MEL.create_material_expression(material, unreal.MaterialExpressionConstant3Vector, 200, 1200)
    grout_color_const.set_editor_property("constant", unreal.LinearColor(GROUT_COLOR[0], GROUT_COLOR[1], GROUT_COLOR[2], 1.0))

    final_color = MEL.create_material_expression(material, unreal.MaterialExpressionLinearInterpolate, 900, 0)
    ...
    grout_rough_const = MEL.create_material_expression(material, unreal.MaterialExpressionConstant, 900, 1350)
    grout_rough_const.set_editor_property("R", 1.0)
    if stone_rough is not None:
        final_rough = MEL.create_material_expression(material, unreal.MaterialExpressionLinearInterpolate, 900, 1500)
    ...
    log("  done -- %s recompiled (cell=%.0fcm, grout_width=%.2f)." % (material.get_name(), CELL_SIZE_CM, GROUT_WIDTH))
