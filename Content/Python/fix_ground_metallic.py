"""
FIX GROUND-LOOKS-LIKE-WATER -- rebuilds M_AI_MountainGround without the ORM metallic channel
================================================================
IRON BREACH / Unreal Engine 5.8

WHAT HAPPENED
-------------
fix_mountains_and_ground.py built M_AI_MountainGround by sampling
T_Iceland_Dirt_ORM and wiring its BLUE channel into Metallic (the standard
R=AO / G=Roughness / B=Metallic "ORM" convention). That texture in this
project apparently isn't packed that way -- the blue channel came back
bright across most of the ground, so the terrain rendered with metallic
pushed way up and roughness down, which is exactly a mirror-like look --
that's the "changed the ground to water" result in your screenshot.

THE FIX
-------
Rather than guess at the correct channel order a second time, this rebuilds
the SAME material (M_AI_MountainGround -- already assigned to your ground
actors, so nothing needs to be re-swapped) using ONLY the BaseColor and
Normal textures, with Metallic left disconnected (defaults to 0 -- dirt/rock
ground should never be metallic anyway) and Roughness set to a fixed 0.9
(matte). No more ORM sampling, no more channel-order guessing.

HOW TO RUN IT
-------------
    py "X:/IronBreach/Content/Python/fix_ground_metallic.py"
...
def log(msg):
    print("[FixGroundMetallic] %s" % msg)


def build_triplanar_basecolor_normal(material, basecolor_tex, normal_tex, tile_world_size=2000.0, blend_sharpness=4.0, roughness=0.9):
    log("Rebuilding %s (basecolor + normal only, no ORM)..." % material.get_name())
    ...
    # Metallic intentionally left disconnected -- defaults to 0. Ground should never be metallic.

    MEL.recompile_material(material)
    log("  done -- %s recompiled (matte, non-metallic)." % material.get_name())
...
def run():
    ...
    if not basecolor:
        log("SKIPPED -- could not load %s" % DIRT_BASECOLOR)
        return
    build_triplanar_basecolor_normal(ground_mat, basecolor, normal)
    log("Done. The ground actors already point at this material, so no actor re-assignment is "
        "needed -- just check the viewport / re-run PIE.")
